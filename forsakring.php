<?php include "head.php"; ?>
	<body>
		<div id="wrap">
<?php include "meny.php" ?>

			<div id="left">
				<h1>Instrumentförsäkring</h1>
				<p class="ingress">Som medlem i Västergötlands Spelmansförbund kan Du försäkra ditt instrument till ett förmånligt pris. Försäkringen som 
				sker i samarbete med Folksam ger ett mycket gott skydd.</p>
				<p> Omfattningen gäller hela världen och täcker skada och förlust. 
				Försäkringen gäller i huvudsak akustiska instrument av alla slag med undantag av flyglar och pianon, där särskilda 
				försäkringar måste tecknas. Nytt avtal mellan SSR och Folksam är tecknat pr 11-2003, kontakta Britt Josefsson för aktuella 
				priser och villkor.</p>
				<p>Om du är intresserad av att teckna försäkring, kolla aktuellt försäkringsvärde. Kontakta därefter Britt Josefsson, som hjälper dig att teckna försäkring.</p>
				<p class="indrag">Tel: 070 – 268 73 55
				E-post: bij1[a]telia.com</p>
				<p>Blankett finns <a href="forsanm2013.doc" target="_blank" title="Blankett">HÄR</a>.
				<a href="http://www.spelmansforbund.org" target="_blank" title="Sveriges Spelmäns Riksförbund">Läs om försäkringen på SSR-s hemsida</a>, klicka på "Försäkring".</p>
				<p>Uppdaterat den 2011-05-30</p>
				<p class="tillbaka"><a href="medlem.php" title="Tillbaka till Medlem">Tillbaka till Medlem</a></p>

			</div>
<?php include "right.php"; ?>
<?php include "sidfot.php" ?>
		</div>
<?php include "script_section.php"; ?>
	</body>
</html>